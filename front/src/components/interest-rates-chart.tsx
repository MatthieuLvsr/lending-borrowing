  ...
  useEffect(() => {
    ...
    const calculateVariableBorrowRate = (utilization: number) => {
      if (utilization <= optimalUtilizationRate) {
        return (
          baseVariableBorrowRate +
          (utilization / optimalUtilizationRate) * variableRateSlope1
        );
      }
      return (
        baseVariableBorrowRate +
    ...
    for (let i = 0; i <= 100; i++) {
      ...
      const y =
        height -
        margin.bottom -
        (Math.min(rate, maxRate) / maxRate) * innerHeight;
    ...
    for (let i = 0; i <= 100; i++) {
      ...
      const y =
        height -
        margin.bottom -
        (Math.min(depositRate, maxRate) / maxRate) * innerHeight;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Visualisation des taux d'intérêt en fonction du taux d'utilisation du
          marché
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="relative aspect-video w-full">
          <canvas
            ...
            width={600}
          />
          <div className="-translate-x-1/2 absolute bottom-8 left-1/2 transform text-muted-foreground text-sm">
